...
from bin.update_file_sizes import update_file_sizes_for_project
from eva_submission.evapro.populate_evapro import EvaProjectLoader
from eva_submission.evapro.table import metadata, Project, Analysis, File
...
class TestUpdateFileSizes(TestCase):
    ...
    def _setup_project_with_files(self, project_accession='PRJEB00001', analysis_accession='ERZ000001',
                                  files_data=None):
        """
        Populate the in-memory DB with a Project, Analysis, and File records linked via junction tables.
        files_data is a list of dicts with keys: filename, file_md5, file_size, file_type (optional).
        Returns the list of File objects created.
        """
    ...
    def test_update_file_size_happy_path(self):
        ...
        with self.patch_evapro_engine(engine):
            ...
            updated, skipped, not_found = update_file_sizes_for_project(self.loader, 'PRJEB00001')
    ...
    def test_md5_mismatch(self):
        """File found by name but MD5 differs -> counted as not_found."""
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        with self.patch_evapro_engine(engine):
            self._setup_project_with_files(
                files_data=[
    ...
    def test_file_not_in_evapro(self):
        """ENA returns a file not present in EVAPRO -> counted as not_found."""
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        with self.patch_evapro_engine(engine):
            self._setup_project_with_files(
                files_data=[
    ...
    def test_project_not_found(self):
        ...
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        with self.patch_evapro_engine(engine):
            updated, skipped, not_found = update_file_sizes_for_project(self.loader, 'NONEXISTENT')
...
class TestUpdateFileSizesFromENA(TestCase):
    ...
    def test_update_file_sizes_from_ena(self):
        ...
        with self.patch_evapro_engine(engine):
            ...
            updated, skipped, not_found = update_file_sizes_for_project(self.loader, project_accession)

            assert updated == len(files_from_ena)
            assert skipped == 0
            assert not_found == 0
