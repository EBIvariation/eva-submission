...
class TestEloadPreparation(TestCase):
    ...
    def setUp(self) -> None:
        ...
        os.chdir(ROOT_DIR)
        self.eload = SubmissionPreparation("submission_1")

    def tearDown(self) -> None:
        SubmissionConfig.content = {}
        eloads = glob.glob(os.path.join(self.resources_folder, 'eloads', 'submission_1'))
        for eload in eloads:
            shutil.rmtree(eload)
        nobackup_eloads = glob.glob(os.path.join(self.resources_folder, 'nobackup_eloads', 'submission_1'))
    ...
    def create_vcfs(self, num_files=2):
        ...
        for i in range(num_files):
            vcf = os.path.join(self.eload.submission_dir, '10_submitted', 'vcf_files', f'file{i}.vcf')
            touch(vcf)
            paths.append(vcf)
        return paths
    ...
    def create_metadata(self, v2=False, num_analyses=0):
        ...
        metadata = os.path.join(self.eload.submission_dir, '10_submitted', 'metadata_file', 'metadata.xlsx')
    ...
    def test_copy_from_ftp(self):
        assert os.listdir(os.path.join(self.eload.submission_dir, '10_submitted', 'vcf_files')) == []
        assert os.listdir(os.path.join(self.eload.submission_dir, '10_submitted', 'metadata_file')) == []
        self.eload.copy_from_ftp(1, 'john')
        assert os.listdir(os.path.join(self.eload.submission_dir, '10_submitted', 'vcf_files')) == ['data.vcf.gz']
        assert os.listdir(os.path.join(self.eload.submission_dir, '10_submitted', 'metadata_file')) == ['metadata.xlsx']
    ...
    def test_detect_submitted_metadata(self):
        ...
        assert self.eload.submission_cfg.query('submission', 'metadata_spreadsheet') == metadata

    def test_detect_submitted_metadata_json(self):
        self.create_vcfs()
        metadata = self.create_metadata(v2=True)
        self.eload.submission_cfg.set('submission', 'metadata_spreadsheet', value=metadata)
        ...
        assert self.eload.submission_cfg.query('submission', 'metadata_json') == metadata_json
    ...
    def test_detect_metadata_attributes(self):
        ...
        assert self.eload.submission_cfg.query('submission', 'project_title') == 'Greatest project ever'
        assert self.eload.submission_cfg.query('submission', 'taxonomy_id') == 9606
        assert self.eload.submission_cfg.query('submission', 'scientific_name') == 'Homo sapiens'
        assert self.eload.submission_cfg.query('submission', 'analyses', 'submission_1_GAE', 'assembly_accession') == 'GCA_000001405.1'
        vcf_files = self.eload.submission_cfg.query('submission', 'analyses', 'submission_1_GAE', 'vcf_files')
    ...
    def test_detect_metadata_attributes_from_json(self):
        ...
        assert self.eload.submission_cfg.query('submission', 'taxonomy_id') == 9606
        assert self.eload.submission_cfg.query('submission', 'scientific_name') == 'Homo sapiens'
        assert self.eload.submission_cfg.query('submission', 'analyses', 'submission_1_GAE',
    ...
    def test_find_genome_single_sequence(self):
        ...
        self.eload.submission_cfg.set('submission', 'scientific_name', value='Thingy thingus')
        # Ensure no other analyses present in the config
        self.eload.submission_cfg.set('submission', 'analyses', value={})
        self.eload.submission_cfg.set('submission', 'analyses', 'Analysis alias test', 'assembly_accession', value='AJ312413.2')
        ...
        with mock.patch("eva_submission.submission_preparation.requests.put", return_value=mock.Mock(status_code=200)):
            ...
            assert self.eload.submission_cfg['submission']['analyses']['Analysis alias test']['assembly_fasta'] \
                   == 'tests/resources/genomes/thingy_thingus/AJ312413.2/AJ312413.2.fa'
            assert self.eload.submission_cfg['submission']['analyses']['Analysis alias test']['assembly_report'] \
                   == 'tests/resources/genomes/thingy_thingus/AJ312413.2/AJ312413.2_assembly_report.txt'

    def test_contig_alias_db_update(self):
        cfg.content['eutils_api_key'] = None
        self.eload.submission_cfg.set('submission', 'scientific_name', value='Thingy thingus')
        self.eload.submission_cfg.set('submission', 'analyses', 'Analysis alias test', 'assembly_accession',
                                      value='GCA_000001405.10')

        with mock.patch("eva_submission.submission_preparation.get_reference_fasta_and_report", return_value=('assembly', 'report')), \
    ...
    def test_convert_new_spreadsheet_to_json(self):
        ...
        self.eload.submission_cfg.set('submission', 'metadata_spreadsheet', value=metadata_copy)
        ...
        with open(json_file, 'r') as f:
            ...
            assert self.eload.submission_cfg.query('submission', 'metadata_json') == json_file
    ...
    def test_update_metadata_json_if_required(self):
        ...
        self.eload.submission_cfg.set('submission', 'metadata_json', value=json_copy)

        # fasta and assembly report determined by find_genome
        self.eload.submission_cfg.set('submission', 'analyses', 'submission_1_VD1', 'assembly_fasta', value='GCA_000009999.9_fasta.fa')
        self.eload.submission_cfg.set('submission', 'analyses', 'submission_1_VD1', 'assembly_report', value='GCA_000009999.9_report.txt')
        self.eload.submission_cfg.set('submission', 'analyses', 'submission_1_VD2', 'assembly_fasta', value='GCA_000001111.1_fasta.fa')
        self.eload.submission_cfg.set('submission', 'analyses', 'submission_1_VD2', 'assembly_report', value='GCA_000001111.1_report.txt')
        ...
        assert 'submission_1/10_submitted/vcf_files' in updated_metadata['files'][0]['fileName']
