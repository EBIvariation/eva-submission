...
cat > valid_deprecations.csv << 'EOF'
assembly_accession,variant_id_file,db_name,deprecation_properties_file
GCA_000001405.2,test_ssids.txt,eva_hsapiens_grch37,test_deprecation_GCA_000001405.2.properties
GCA_000001405.3,test_ssids.txt,eva_hsapiens_grch38,test_deprecation_GCA_000001405.3.properties
EOF

# Create minimal properties files for both assemblies
touch test_deprecation_GCA_000001405.2.properties
touch test_deprecation_GCA_000001405.3.properties
...
printf "\e[32m====== Log files created ======\e[0m\n"
ls logs/
...
rm -f test_ssids.txt drop_study.properties valid_deprecations.csv
rm -f test_deprecation_GCA_000001405.2.properties test_deprecation_GCA_000001405.3.properties
...
if [ "${DROP_COUNT}" -ne 0 ]; then
    echo "ERROR: Expected 0 drop_study processes, got ${DROP_COUNT}"
    exit 1
fi
...
rm -rf work .nextflow* logs
rm -f test_ssids.txt drop_study.properties valid_deprecations.csv
rm -f test_deprecation_GCA_000001405.2.properties
