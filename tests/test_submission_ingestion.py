...
from eva_submission.submission_ingestion import SubmissionIngestion, project_dirs
...
class TestEloadIngestion(TestCase):
    ...
    def setUp(self):
        ...
        with patch('eva_submission.submission_ingestion.get_mongo_uri_for_eva_profile', autospec=True):
            self.eload = SubmissionIngestion('submission_33')
        # Used to restore test config after each test
        self.original_cfg = deepcopy(self.eload.submission_cfg.content)

    def tearDown(self):
        for proj_dir_name in project_dirs.values():
            proj_dir_path = os.path.join(self.eload.submission_dir, proj_dir_name)
        ...
        for ingest_file_name in ingest_files:
            ingest_file_path = os.path.join(self.eload.submission_dir, ingest_file_name)
            if os.path.exists(ingest_file_path):
                os.remove(ingest_file_path)
        self.eload.submission_cfg.content = self.original_cfg

    def _patch_get_dbname(self, db_name):
        m_get_db_name = patch(
            'eva_submission.submission_ingestion.resolve_variant_warehouse_db_name',
            autospec=True,
    ...
    def _patch_mongo_database(self, collection_names=None):
        ...
        return patch('eva_submission.submission_utils.MongoDatabase', autospec=True, return_value=mongodb_instance)

    def test_check_brokering_done(self):
        self.eload.project_accession = None
        with self.assertRaises(ValueError):
            self.eload.check_brokering_done()
        del self.eload.submission_cfg.content['brokering']
        with self.assertRaises(ValueError):
            self.eload.check_brokering_done()

    def test_check_variant_db_no_creation(self):
        with self._patch_metadata_handle(), self._patch_get_dbname('eva_ecaballus_30'), \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
            ...
            self.assertEqual(
                'eva_ecaballus_30',
                self.eload.submission_cfg.query('ingestion', 'database', 'GCA_002863925.1', 'db_name')
            )
    ...
    def test_load_from_ena(self):
        with self._patch_metadata_handle(), \
                ...
                patch('eva_submission.submission_ingestion.check_project_exists_in_evapro'), \
    ...
    def test_ingest_all_tasks(self):
        with self._patch_metadata_handle(), \
                self._patch_metadata_engine(), \
                patch.object(EvaProjectLoader, 'refresh_study_browser'), \
                patch('eva_submission.submission_ingestion.get_all_results_for_query') as m_get_results, \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True), \
                patch('eva_submission.submission.get_hold_date_from_ena') as m_get_hold_date, \
                patch('eva_submission.submission_ingestion.get_vep_and_vep_cache_version') as m_get_vep_versions, \
                patch('eva_submission.submission_ingestion.get_species_name_for_assembly') as m_get_species, \
                patch('eva_submission.submission_ingestion.get_assembly_name_and_taxonomy_id') as m_get_tax, \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
    ...
    def test_ingest_metadata_load(self):
        with self._patch_metadata_handle(), \
                ...
                patch.object(EvaProjectLoader, 'refresh_study_browser'), \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True), \
                patch('eva_submission.submission_utils.get_project_alias') as m_get_alias_results, \
                patch('eva_submission.submission_utils.requests.post') as m_post, \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
                patch.object(EvaProjectLoader, 'load_project_from_ena'), \
                patch.object(EvaProjectLoader, 'load_samples_from_vcf_file'), \
                patch.object(EvaProjectLoader, 'update_project_samples_temp1'), \
    ...
    def test_ingest_accession(self):
        with self._patch_metadata_handle(), \
            ...
            assert os.path.exists(
                os.path.join(self.eload.submission_dir, 'accession_and_load_params.yaml')
            )
    ...
    def assert_vep_versions(self, vep_version, vep_cache_version, vep_species):
        ingest_csv = os.path.join(self.eload.submission_dir, 'vcf_files_to_ingest.csv')
    ...
    def test_ingest_variant_load_vep_versions_found(self):
        with self._patch_metadata_handle(), \
                patch.object(SubmissionIngestion, '_update_metadata_post_ingestion') as m_post_load_metadata, \
                patch('eva_submission.submission_ingestion.get_all_results_for_query') as m_get_results, \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True), \
                patch('eva_submission.submission_utils.get_project_alias') as m_get_alias_results, \
                patch('eva_submission.submission_ingestion.get_vep_and_vep_cache_version') as m_get_vep_versions, \
    ...
    def test_ingest_variant_load_vep_versions_found_from_evapro(self):
        ...
        with self._patch_metadata_handle(), \
                ...
                patch('eva_submission.submission_utils.get_project_alias') as m_get_alias_results, \
                patch('eva_submission.submission_ingestion.get_vep_and_vep_cache_version') as m_get_vep_versions, \
                patch('eva_submission.submission_utils.get_taxonomy_id_and_name_of_assembly_from_evapro') as m_get_evapro_tax, \
                ...
                patch('eva_submission.submission_utils.retrieve_species_scientific_name_from_tax_id_ncbi') as m_get_ncbi_name, \
                patch('eva_submission.submission_utils.requests.post') as m_post, \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
                self._patch_mongo_database():
            m_get_alias_results.return_value = 'alias'
    ...
    def test_ingest_clustering(self):
        with self._patch_metadata_handle(), \
                ...
                patch.object(EvaProjectLoader, 'refresh_study_browser'), \
                patch('eva_submission.submission_ingestion.get_all_results_for_query') as m_get_results, \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True) as m_run_command, \
                patch('eva_submission.submission_ingestion.get_assembly_name_and_taxonomy_id') as m_get_tax, \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
                self._patch_mongo_database():
            ...
            assert self.eload.submission_cfg.query('ingestion', 'remap_and_cluster', 'target_assembly') == 'GCA_123'
            assert m_run_command.call_count == 1

    def test_ingest_clustering_no_supported_assembly(self):
        with self._patch_metadata_handle(), \
                patch('eva_submission.submission_ingestion.get_all_results_for_query') as m_get_results, \
                patch.object(SubmissionIngestion, '_get_target_assembly') as m_target_assembly, \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True) as m_run_command, \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
                self._patch_mongo_database():
            ...
            self.eload.ingest(tasks=['optional_remap_and_cluster'])
            assert self.eload.submission_cfg.query('ingestion', 'remap_and_cluster', 'target_assembly') is None
    ...
    def test_ingest_clustering_supported_assembly_in_another_taxonomy(self):
        with self._patch_metadata_handle(), \
                ...
                patch.object(EvaProjectLoader, 'refresh_study_browser'), \
                patch('eva_submission.submission_ingestion.get_all_results_for_query') as m_get_results, \
                patch.object(SubmissionIngestion, "_get_supported_assembly_from_evapro", new=MagicMock()) as m_get_supported_asm, \
                patch.object(SubmissionIngestion, "_insert_new_supported_asm_from_ensembl", new=MagicMock()) as m_new_supported_asm, \
                patch('eva_submission.submission_ingestion.get_assembly_name_and_taxonomy_id') as m_get_tax, \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True) as m_run_command, \
                patch('eva_submission.submission_ingestion.insert_new_assembly_and_taxonomy') as insert_asm_tax, \
                self._patch_mongo_database():
            ...
            m_get_results.return_value = []
            self.eload.ingest(tasks=['optional_remap_and_cluster'])
            assert self.eload.submission_cfg.query('ingestion', 'remap_and_cluster', 'target_assembly') == 'gca_in_another_tax'
        with self._patch_metadata_handle(), \
                patch('eva_submission.submission_ingestion.get_all_results_for_query') as m_get_results, \
                patch.object(SubmissionIngestion, "_get_supported_assembly_from_evapro", new=MagicMock()) as m_get_supported_asm, \
                patch.object(SubmissionIngestion, "_insert_new_supported_asm_from_ensembl", new=MagicMock()) as m_new_supported_asm, \
    ...
    def test_resume_when_step_fails(self):
        with self._patch_metadata_handle(), \
            ...
            for task in ['accession', 'variant_load']:
                nextflow_dir = self.eload.submission_cfg.query(self.eload.config_section, 'accession_and_load', 'nextflow_dir', task)
                assert os.path.exists(nextflow_dir)

            self.eload.ingest(resume=True)
            for task in ['accession', 'variant_load']:
                assert self.eload.submission_cfg.query(self.eload.config_section, 'accession_and_load', 'nextflow_dir', task) == '<complete>'
    ...
    def test_resume_with_tasks(self):
        with self._patch_metadata_handle(), \
            ...
            accession_nextflow_dir = self.eload.submission_cfg.query(self.eload.config_section, accession_config_section,
            ...
            new_remap_nextflow_dir = self.eload.submission_cfg.query(self.eload.config_section, remap_config_section, 'nextflow_dir', 'optional_remap_and_cluster')
            ...
            self.eload.ingest(tasks=['accession'], resume=True)
            new_accession_nextflow_dir = self.eload.submission_cfg.query(self.eload.config_section, accession_config_section,
                                                                    'nextflow_dir', 'accession')
    ...
    def test_get_target_assembly_fallback_on_submitted_assembly(self):
        with self._patch_metadata_handle(), \
                patch('eva_submission.submission_ingestion.get_all_results_for_query'), \
                patch.object(SubmissionIngestion, '_insert_new_supported_asm_from_ensembl') as m_ensembl_asm:
    ...
    def _patch_pre_run_nextflow(self):
        return patch.object(SubmissionIngestion, 'create_nextflow_temp_output_directory', return_value='work_dir'), \
                patch('eva_submission.submission_ingestion.command_utils.run_command_with_output', autospec=True), \
                patch('eva_submission.submission_ingestion.shutil.rmtree')
    ...
    def _post_run_nextflow_assert(self, m_run_command, workflow_name, work_dir, resume, task_performed, task_completed):
        for task in task_completed:
            assert self.eload.submission_cfg.query('ingestion', workflow_name, 'nextflow_dir', task) == '<complete>'
    ...
    def test_run_nextflow_resume(self):
        ...
        with p_cr, p_cmd as m_run_command, p_rm:
            self.eload.submission_cfg.set('ingestion', workflow_name, 'nextflow_dir', 'task1', value='<complete>')
            self.eload.submission_cfg.set('ingestion', workflow_name, 'nextflow_dir', 'task2', value=nextflow_dir)
    ...
    def test_run_nextflow_resume_incompatible(self):
        ...
        tasks = ['task1', 'task2']
        with p_cr, p_cmd as m_run_command, p_rm:
            self.eload.submission_cfg.set('ingestion', workflow_name, 'nextflow_dir', 'task1', value=nextflow_dir1)
            self.eload.submission_cfg.set('ingestion', workflow_name, 'nextflow_dir', 'task2', value=nextflow_dir2)
            self.eload.run_nextflow(workflow_name, param_values, resume=True, tasks=tasks)
