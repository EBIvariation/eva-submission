...
class TestEloadValidation(TestCase):
    ...
    def setUp(self) -> None:
        ...
        os.chdir(ROOT_DIR)
        self.validation = SubmissionValidation('submission_2')
        self.sv_validation = SubmissionValidation('submission_70')
        # Used to restore test config after each test
        self.original_cfg = deepcopy(self.validation.submission_cfg.content)
        self.original_sv_cfg = deepcopy(self.sv_validation.submission_cfg.content)

    def tearDown(self):
        self.validation.submission_cfg.content = self.original_cfg
        self.sv_validation.submission_cfg.content = self.original_sv_cfg
...
General details about the project
	...
	Submission Directory: eloads/submission_2
	Files mapping:
		---
...
'''
    ...
    def test_mark_valid_files_and_metadata(self):
        assert self.validation.submission_cfg.query('validation', 'valid') is None
        self.validation.mark_valid_files_and_metadata()
        # Check that the normalised file was picked up instead of the original file
        expected = {'analyses': {'submission_2_analysis_alias': {'vcf_files': ['test.vcf']}},
                    'metadata_spreadsheet': '/path/to/the/spreadsheet'}
        assert self.validation.submission_cfg.query('validation', 'valid') == expected
    ...
    def test_update_config_with_cli_results(self):
        ...
        expected_aggregation = {
            'submission_2_Analysis A': None,
            'submission_2_Analysis B': 'basic',
        }
        assert self.validation.submission_cfg.query('validation', 'aggregation_check', 'analyses') == expected_aggregation
        assert self.validation.submission_cfg.query('validation', 'vcf_check')['pass'] == False
        assert self.validation.submission_cfg.query('validation', 'assembly_check')['pass'] == True
        assert self.validation.submission_cfg.query('validation', 'metadata_check')['pass'] == True
        assert self.validation.submission_cfg.query('validation', 'sample_check')['pass'] == True

    def test_set_validation_task_result_valid(self):
        self.validation.submission_cfg.set('validation', 'vcf_check', 'pass', value=False)
        self.validation.submission_cfg.set('validation', 'assembly_check', 'pass', value=True)
        del self.validation.submission_cfg['validation']['metadata_check']
        ...
        assert self.validation.submission_cfg.query('validation', 'vcf_check')['pass'] == False
        assert self.validation.submission_cfg.query('validation', 'vcf_check')['forced'] == True

        assert self.validation.submission_cfg.query('validation', 'assembly_check')['pass'] == True
        assert 'forced' not in self.validation.submission_cfg.query('validation', 'assembly_check')

        assert 'pass' not in self.validation.submission_cfg.query('validation', 'metadata_check')
        assert self.validation.submission_cfg.query('validation', 'metadata_check')['forced'] == True
