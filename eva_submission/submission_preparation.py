...
from eva_sub_cli_processing.sub_cli_utils import sub_ws_url_build, put_to_sub_ws
from eva_submission.biosample_submission.biosamples_submitters import get_biosample_characteristics
from eva_submission.submission import Submission, directory_structure
from eva_submission.submission_utils import resolve_accession_from_text, get_reference_fasta_and_report, NCBIAssembly, \
...
class SubmissionPreparation(Submission):

    def copy_from_ftp(self, ftp_box, submitter):
        box = FtpDepositBox(ftp_box, submitter)

        vcf_dir = os.path.join(self.submission_dir, directory_structure['vcf'])
        ...
        if box.most_recent_metadata:
            ...
            metadata_dir = os.path.join(self.submission_dir, directory_structure['metadata'])
    ...
    def replace_values_in_metadata(self, taxid=None, reference_accession=None):
        ...
        input_spreadsheet = self.submission_cfg.query('submission', 'metadata_spreadsheet')
        json_file = self.submission_cfg.query('submission', 'metadata_json')
        if json_file:
    ...
    def detect_submitted_metadata(self):
        ...
        if len(metadata_json) == 1:
            self.submission_cfg.set('submission', 'metadata_json', value=metadata_json[0])
        elif len(metadata_spreadsheets) == 1:
            self.submission_cfg.set('submission', 'metadata_spreadsheet', value=metadata_spreadsheets[0])
        else:
    ...
    def check_submitted_filenames(self):
        ...
        json_data = None
        eva_files_json = self.submission_cfg.query('submission', 'metadata_json')
        if eva_files_json:
            ...
            eva_files_sheet = self.submission_cfg.query('submission', 'metadata_spreadsheet')
    ...
    def detect_metadata_attributes_from_spreadsheet(self):
        eva_metadata = EvaXlsxReader(self.submission_cfg.query('submission', 'metadata_spreadsheet'))
        analysis_reference = {}
        ...
        for file in eva_metadata.files:
            if is_vcf_file(file.get('File Name')):
                file_full = os.path.join(self.submission_dir, directory_structure['vcf'], file.get("File Name"))
                analysis_alias = self._unique_alias(file.get("Analysis Alias"))
                analysis_reference[analysis_alias]['vcf_files'].append(file_full)
        self.submission_cfg.set('submission', 'analyses', value=analysis_reference)

        self.submission_cfg.set('submission', 'project_title', value=eva_metadata.project_title)

        taxonomy_id = eva_metadata.project.get('Tax ID')
        if taxonomy_id and (isinstance(taxonomy_id, int) or taxonomy_id.isdigit()):
            self.submission_cfg.set('submission', 'taxonomy_id', value=int(taxonomy_id))
            scientific_name = get_scientific_name(taxonomy_id)
            self.submission_cfg.set('submission', 'scientific_name', value=scientific_name)
        else:
            if taxonomy_id:
    ...
    def detect_metadata_attributes_from_json(self):
        with open(self.submission_cfg.query('submission', 'metadata_json'), 'r') as open_file:
            json_data = json.load(open_file)
        ...
        for file_entry in json_data.get('files'):
            if is_vcf_file(file_entry.get('fileName')):
                file_full = os.path.join(self.submission_dir, directory_structure['vcf'], os.path.basename(file_entry.get("fileName")))
                analysis_alias = self._unique_alias(file_entry.get("analysisAlias"))
                analysis_reference[analysis_alias]['vcf_files'].append(file_full)
        self.submission_cfg.set('submission', 'analyses', value=analysis_reference)

        self.submission_cfg.set('submission', 'project_title', value=json_data.get('project').get('title'))
    ...
    def find_genome(self):
        ...
        if scientific_name:
            for analysis_alias in analyses:
                assembly_accession = self.submission_cfg.query('submission', 'analyses', analysis_alias, 'assembly_accession')
                ...
                self.submission_cfg.set('submission', 'analyses', analysis_alias, 'assembly_report', value=assembly_report_path)
                self.submission_cfg.set('submission', 'analyses', analysis_alias, 'assembly_fasta', value=assembly_fasta_path)
    ...
    def update_metadata_json_if_required(self):
        ...
        metadata_json_path = self.submission_cfg.query('submission', 'metadata_json')
        ...
        analyses_in_config = self.submission_cfg.query('submission', 'analyses')
        ...
        for file in metadata_json['files']:
            file['fileName'] = os.path.join(self.submission_dir, directory_structure['vcf'],
    ...
    def convert_new_spreadsheet_to_json(self):
        metadata_xlsx = self.submission_cfg.query('submission', 'metadata_spreadsheet')
    ...
    def add_submission_id_to_config(self):
        self.submission_cfg.set('submission', 'submission_id', value=self.submission_id)

    def link_submission_id_and_eload(self, submission_id, eload_num, source):
        json_data = {
            "eload": eload_num,
