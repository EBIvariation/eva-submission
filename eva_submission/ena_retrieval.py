...
def get_file_from_md5(md5):
    with get_metadata_connection_handle(cfg['maven']['environment'], cfg['maven']['settings_file']) as conn:
        ...
        if len(file_ids) > 1:
            raise Exception(f'Multiple file found with md5 {md5}')
        elif len(file_ids) == 0:
            return None
        return file_ids[0]
        return file_ids
...
def difference_evapro_file_set_with_ena_for_analysis(analysis_accession, ena_list_of_file_dicts):
    ...
    with get_metadata_connection_handle(cfg['maven']['environment'], cfg['maven']['settings_file']) as conn:
        ...
        set_of_file_from_ena = [d.get('md5') for d in ena_list_of_file_dicts]
        set_of_file_from_eva_pro = [d.get('md5') for d in eva_list_of_file_dicts]
        if set_of_file_from_ena != set_of_file_from_eva_pro:
            ...
            for filename, md5 in file_specific_to_ena:
                logger.warn(f'File {filename} exist in ENA but not in EVA.')
            file_specific_to_eva = set_of_file_from_eva_pro.difference(set_of_file_from_ena)
            for filename, md5 in file_specific_to_eva:
                logger.warn(f'File {filename} exist in EVA but not in ENA.')
            return [file_dict for file_dict in set_of_file_from_ena if file_dict['md5'] in file_specific_to_ena],\
                   [file_dict for file_dict in set_of_file_from_eva_pro if file_dict['md5'] in file_specific_to_eva]
    return None, None
