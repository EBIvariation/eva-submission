...
def get_or_generate_submission_id_for_eload(eload_id, source='email'):
    response = get_from_sub_ws(sub_ws_url_build('admin', 'submission', str(eload_id), 'submissionId', source=source))
    if not response:
        return None
